...
const SERVER_ADDRESS = "http://127.0.0.1:7777";

async function getCatalog(categoryNameEscaped) {
    try {
        const response = await fetch(`${SERVER_ADDRESS}/catalog?category=${categoryNameEscaped}`);
    ...
    if (errorCode === 200) {
        ...
        if (categoryList.length > 0) {
            ...
            categoryList.forEach(element => {
                ...
                const imageUrl = `${SERVER_ADDRESS}/${element}`;
